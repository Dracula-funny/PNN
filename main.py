...
def train(config):
    ...
    if config.MODEL.PRETRAINED:
        ...
        knn_acc, proto_acc, acc, loss = validate(config, valid_dataset, valid_dataloader, model)
        logger.info(f"Accuracy of the network on the {len(valid_dataloader)} test images: {acc:.1f}%")
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        step = train_one_epoch(config, model, train_dataset, train_dataloader, optimizer, epoch, lr_scheduler, step,
                               writer)
        knn_acc, proto_acc, acc_current, loss = validate(config, valid_dataset, valid_dataloader, model, epoch, writer)
...
def test(config):
    ...
    # with torch.no_grad():
    knn_acc, proto_acc, acc1, loss, ci = testing(config, test_dataset, test_dataloader,  model,optimizer,lr_scheduler,step)
    logger.info(
        f"Test Accuracy of {config.DATA.TEST.DATASET_NAMES[0]}: {acc1:.2f}%+-{ci:.2f}")
...
def train_one_epoch(config, model, dataset, data_loader, optimizer, epoch, lr_scheduler, step, writer):
    ...
    for idx, batches in enumerate(data_loader):
        ...
        loss, knn_acc, proto_acc, acc = model.train_forward(imgs, labels, dataset_index)
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            if not ((not config.DATA.TRAIN.IS_EPISODIC) and config.DATA.TRAIN.ITERATION_PER_EPOCH is None and len(
                    config.DATA.TRAIN.DATASET_NAMES) > 1):
...
def testing(config, dataset, data_loader, model,optimizer=None,lr_scheduler=None,step=None):
    ...
    for idx, batches in enumerate(data_loader):
        ...
        loss,  acc= model.test_forward(imgs, labels, dataset_index,model = model,optimizer = optimizer,step = step)
